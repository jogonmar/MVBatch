...
switch data_type,
    case 1,
        ...
        for k=1:max_var:s2,
           % hnd = [hnd figure];
            indj = k:min(s2,k+max_var-1);
            lenj = length(indj);
            for j=1:lenj,
                subplot(rows,columns,j);
                maxy = max(x(1).data{ind_v(indj(j))}(:,ind_v2(indj(j))));
                miny = min(x(1).data{ind_v(indj(j))}(:,ind_v2(indj(j)))); 
                hold on
                for i=1:max(s),
                    plot(x(i).data{ind_v(indj(j))}(:,1),x(i).data{ind_v(indj(j))}(:,ind_v2(indj(j))))
                    maxk(j) = max(maxk(j),x(i).data{ind_v(indj(j))}(end,1));
                    maxy = max(maxy,max(x(i).data{ind_v(indj(j))}(:,ind_v2(indj(j)))));
                    miny = min(miny,min(x(i).data{ind_v(indj(j))}(:,ind_v2(indj(j)))));

                end
                if tt,
                    st = size(test);                   
                    for i=1:max(st),
                        plot(test(i).data{ind_v(indj(j))}(:,1),test(i).data{ind_v(indj(j))}(:,ind_v2(indj(j))),'r-.')
                        maxk(j) = max(maxk(j),test(i).data{ind_v(indj(j))}(end,1));
                        maxy = max(maxy,max(test(i).data{ind_v(indj(j))}(:,ind_v2(indj(j)))));
                        miny = min(miny,min(test(i).data{ind_v(indj(j))}(:,ind_v2(indj(j)))));
    ...
    case 2,
        ...
        for k=1:max_var:s2(2),
            ...
            for j=1:lenj,
                ...
                for i=1:max(s),
                    plot(x{i}(:,indj(j)))
                    maxk = max(maxk,size(x{i},1));
                    maxy = max(maxy,max(x{i}(:,indj(j))));
                    miny = min(miny,min(x{i}(:,indj(j))));

                end
                if tt,
                    st = size(test);
                    for i=1:max(st),
                        plot(test{i}(:,indj(j)),'r-.')
                        maxk = max(maxk,size(test{i},1));
                        maxy = max(maxy,max(test{i}(:,indj(j))));
                        miny = min(miny,min(test{i}(:,indj(j))));
        ...
        for k=1:max_var:s(2),
            ...
            for j=1:lenj,
                ...
                if tt,
                    ...
                    m = min(m,min(min(test(:,indj(j),:))));
                    M = max(M,max(max(test(:,indj(j),:))));
                end
                ...
                axis([1 s(1) m M]); % Cambiar por m y M
