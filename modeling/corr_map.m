function map = corr_map(x,c_max,type,p_min,opt,console)

% Computes and plots batch-wise correlation map.
%
% map = corr_map(x)          % total correlation map
% map = corr_map(x,c_max,type,p_min,opt) % output in MATLAB console 
% map = corr_map(x,c_max,type,p_min,opt,console)  % complete call
%
%
% INPUTS:
%
...
%
% type: (1x1) type of correlation map.
%       1: total correlation map (by default).
%       2: dynamic partial correlation map (pseudoinverse). 
%       3: instantaneous+dynamic partial correlation map (pseudoinverse). 
%
...
% OUTPUTS:
%
% map: (J·KxJ·K) correlation map.
%
%
% coded by: Jose Camacho Paez (josecamacho@ugr.es)
% last modification: 02/May/17
%
...
if nargin < 6, console = 0; end;

% Computation

xu = preprocess2D(unfold(x,Inf),2);
s = size(xu);
so = size(x);
...
switch type,
    ...
    case 1, % Total Correlation Map
        map = (xu'*xu)/(s(1)-1);
        tit = 'Correlation Map';
 
    case 2, % MV-wise Partial (Backwards) Correlation Map using the Pseudo-inverse adjusting both for dynamic relationships
        tit = 'Dynamic Partial (Backwards) Correlation Map';
        map = (xu'*xu)/(s(1)-1);
    ...
    case 3, % MV-wise Partial (Backwards) Correlation Map using the Pseudo-inverse adjusting both for dynamic and instantaneous relationships
        tit = 'Partial (Backwards) Correlation Map';
        map = (xu'*xu)/(s(1)-1);
        eu = xu; % backward regression
        au = xu; % fordward regression
