function [mp_model,text_tot] = mppca2_s(x,lag,T,absolute,gamma,minsize,n,prep,cross,console,text_tot)

% Top-Down step of MPPCA, version 2. Division in the samples of the
% unfolded data. 
%
% [mp_model,text_tot] = mppca2_s(x) % variable-wise MP model with standard parameters
% [mp_model,text_tot] = mppca2_s(x,lag) % batch dynamic MP model with standard parameters
% [mp_model,text_tot] = mppca2_s(x,lag,T,absolute,gamma,minsize,n,prep,cross) % output
%                                                           in MATLAB console
% [mp_model,text_tot] = mppca2_s(x,lag,T,absolute,gamma,minsize,n,prep,cross,console,text_tot) % complete call
%
%
% INPUTS:
%
...
[mp_model,text_tot] = high_mppca2_s(arg,clu,pc,cumpress,console,text_tot);
